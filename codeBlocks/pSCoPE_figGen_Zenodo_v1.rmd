...
# Borrowed from SCoPE2 (Specht, et al.,)
...
for (i in 1:length(unique(GO_db.2$GO_term_name))){
  GOTerm.single.name <- unique(GO_db.2$GO_term_name)[i]
...
for (i in 1:length(GOtermsForCor)){
  DB.lim <- GO_db.2 %>% filter(GO_term_name %in% unique(GOtermsForCor)[i])
...
Scatter
ggsave("ScatterRevised_supplemental_v2.png", width =11, height = 11, units = "in")

ProcessOut$MinimalLabels <- ifelse(ProcessOut$Process == "type I interferon-mediated signaling pathway","Type I IFN signaling",ifelse(ProcessOut$Process == "proton transport","Proton transport", "Other GO terms"))
...
ScatterFig4 <- ProcessOut %>% filter(Process %in% selectedProcessForPlot)  %>% arrange(rev(Process)) %>%
  ggplot(aes(x=scRat,y =bulkRat, fill = factor(MinimalLabels, levels = c("Proton transport","Type I IFN signaling","Other GO terms")), size = Size)) + 
  geom_point(shape = 21, stroke = 1.5, alpha =0.5) + 
  theme_light() + labs(x = expression("Single Cells"), y = expression("Bulk")) +
  theme(legend.position = c(0.38,0.82), 
        legend.title = element_blank(),
        legend.text = element_text(size = 18),
        legend.background = element_rect(fill = "transparent"),
...
ScatterFig4
ggsave("ScatterRevised_Fig4c.pdf", width =5, height = 5, units = "in", device = cairo_pdf)
...
ProtonPumpPair
ggsave("ProtonPumpPair.pdf", width = 7, height = 5, units = "in", device = cairo_pdf)
...
ggsave("ProtonPump_CorHeat_Supplemental.pdf", width = 11, height = 7.33, units = "in", device = cairo_pdf)
