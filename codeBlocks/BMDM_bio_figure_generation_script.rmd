...
```{r Missing data PCA: precursor-level}
...
#ggsave(paste0(figOut,"DataCompleteness_PCA_supplemental.pdf"), width = 7, height = 8, units = "in")
ggsave(paste0("C:/My_Drive/MS/SCoPE/t-SCoPE/code/pSCoPE_pubFigs/Figs/v2_figs/SupplementalFigs/","DataCompleteness_PCA_supplemental.pdf"), width = 7, height = 7, units = "in")
```

```{r Missing data PCA: precursor-level}

PMD.m <- melt(ProtMissing, id.vars = "Proteins")
PMD.m_sum <- PD.m %>% group_by(variable) %>% summarize(fracMD = sum(is.na(value))/n())
