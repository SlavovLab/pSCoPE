---
title: "pSCoPE_figGen_Final_1022"
author: "Gray Huffman"
date: '2022-10-04'
...
## pSCoPE figure generation script
...
datIn <- "C:/MyMSMS/SCoPE/t-SCoPE/code/pSCoPE_FigGen_1022/datIn/"

FigOut <- "C:/MyMSMS/SCoPE/t-SCoPE/code/pSCoPE_FigGen_1022/FigOut/"
datOut <- "C:/MyMSMS/SCoPE/t-SCoPE/code/pSCoPE_FigGen_1022/DatOut/"
```
...
###
# Rebuttal Figure 1 plot
###




CollectedContrast %>% filter(Platform == "Prioritized") %>% ggplot(aes(value*100,y = legendTitle ,color = legendTitle)) + geom_boxplot(lwd = 1) + 
   geom_point(position=position_jitterdodge(), size = 2, shape = 21, fill = "grey95") + 
...
#ggsave(paste0(FigOut,"Coverage_Rebuttal_rev2_1pFDRMS2_forLegend.pdf"), width = 7.2 , height = 5.1, units = "in")


#msmsS.cov_Productive_sum_v3.fSUM <- msmsS.cov_Productive_sum_v3.f %>% group_by(Method) %>% summarize(medVal = median(numObs, na.rm = TRUE))


#evi.cov_PSMs_sum.f_sum <- evi.cov_PSMs_sum.f %>% group_by(Method) %>% summarize(medianVal = median(numObs))

#MDp_join_sumSUM <- MDp_join_sum %>% group_by(Method) %>% summarize(medianVal = median(numProts))

#(((msmsS.cov_Productive_sum_v3.fSUM [1,2]*100) - (msmsS.cov_Productive_sum_v3.fSUM [2,2]*100))/(msmsS.cov_Productive_sum_v3.fSUM [2,2]*100))*100

#(((evi.cov_PSMs_sum.f_sum[1,2]*100) - (evi.cov_PSMs_sum.f_sum[2,2]*100))/(evi.cov_PSMs_sum.f_sum[2,2]*100))*100

#(((MDp_join_sumSUM[1,2]*100) - (MDp_join_sumSUM[2,2]*100))/(MDp_join_sumSUM[2,2]*100))*100
```
...
scp_filt.ev_shotgun <- scp_filt.ev %>% filter(Raw.file %in% paste0("X",ShotgunSet)) %>% group_by(modseq) %>% slice(1)
...
#PeptideMD
#ggsave(paste0(FigOut,"PepMD_consistency_Rebuttal_fig_v4_completeness.pdf"), width = 5 , height = 5.1, units = "in")
...
#SI <- read.delim("C:/MyMSMS/SCoPE/t-SCoPE/dat/Rebut_SpikeInPepSets_081522/SC/evidence.txt")
...
yIndex <- seq(50,400,(400-50)/7)
offset <- 6.5
SI.seqCharge %>% ggplot(aes(log10(Intensity))) + coord_flip(ylim = c(0,500)) + 
  geom_segment(y =0, yend = yIndex[1], 
    x = log10(SI.seqCharge.SI$Intensity[1]), xend = log10(SI.seqCharge.SI$Intensity[1]), 
    color = SI.seqCharge.SI$color[1], size = .5) +
    geom_segment(y =0, yend = yIndex[2], 
    x = log10(SI.seqCharge.SI$Intensity[2]), xend = log10(SI.seqCharge.SI$Intensity[2]),
    color = SI.seqCharge.SI$color[2], size = .5) +
    geom_segment(y =0, yend = yIndex[3], 
    x = log10(SI.seqCharge.SI$Intensity[3]), xend = log10(SI.seqCharge.SI$Intensity[3]),
    color = SI.seqCharge.SI$color[3], size = .5) +
    geom_segment(y =0, yend = yIndex[4], 
    x = log10(SI.seqCharge.SI$Intensity[4]), xend = log10(SI.seqCharge.SI$Intensity[4]),
    color = SI.seqCharge.SI$color[4], size = .5) +
    geom_segment(y =0, yend = yIndex[5], 
    x = log10(SI.seqCharge.SI$Intensity[5]), xend = log10(SI.seqCharge.SI$Intensity[5]),
    color = SI.seqCharge.SI$color[5], size = .5) +
    geom_segment(y =0, yend = yIndex[6], 
    x = log10(SI.seqCharge.SI$Intensity[6]), xend = log10(SI.seqCharge.SI$Intensity[6]),
    color = SI.seqCharge.SI$color[6], size = .5) +
    geom_segment(y =0, yend = yIndex[7], 
    x = log10(SI.seqCharge.SI$Intensity[7]), xend = log10(SI.seqCharge.SI$Intensity[7]),
    color = SI.seqCharge.SI$color[7], size = .5) +
    geom_segment(y =0, yend = yIndex[8], 
    x = log10(SI.seqCharge.SI$Intensity[8]), xend = log10(SI.seqCharge.SI$Intensity[8]),
    color = SI.seqCharge.SI$color[8], size = .5) +
    annotate("text",x = log10(SI.seqCharge.SI$Intensity[1])+.1, y = yIndex[1]-offset,hjust = 0, 
             label = SI.seqCharge.SI$SeqCharge[1], angle = 45, color = SI.seqCharge.SI$color[1], fontface = 2) +
    annotate("text",x = log10(SI.seqCharge.SI$Intensity[2])+ .1, y = yIndex[2]-offset,hjust = 0, 
             label = SI.seqCharge.SI$SeqCharge[2], angle = 45, color = SI.seqCharge.SI$color[2], fontface = 2) +
    annotate("text",x = log10(SI.seqCharge.SI$Intensity[3])+ .1, y = yIndex[3]-offset,hjust = 0, 
             label = SI.seqCharge.SI$SeqCharge[3], angle = 45, color = SI.seqCharge.SI$color[3], fontface = 2) +
    annotate("text",x = log10(SI.seqCharge.SI$Intensity[4])+ .1, y = yIndex[4]-offset,hjust = 0, 
             label = SI.seqCharge.SI$SeqCharge[4], angle = 45, color = SI.seqCharge.SI$color[4], fontface = 2) +
    annotate("text",x = log10(SI.seqCharge.SI$Intensity[5])+ .1, y = yIndex[5]-offset,hjust = 0, 
             label = SI.seqCharge.SI$SeqCharge[5], angle = 45, color = SI.seqCharge.SI$color[5], fontface = 2) +
    annotate("text",x = log10(SI.seqCharge.SI$Intensity[6])+ .1, y = yIndex[6]-offset,hjust = 0, 
             label = SI.seqCharge.SI$SeqCharge[6], angle = 45, color = SI.seqCharge.SI$color[6], fontface = 2) +
    annotate("text",x = log10(SI.seqCharge.SI$Intensity[7])+ .1, y = yIndex[7]-offset,hjust = 0, 
             label = SI.seqCharge.SI$SeqCharge[7], angle = 45, color = SI.seqCharge.SI$color[7], fontface = 2) +
    annotate("text",x = log10(SI.seqCharge.SI$Intensity[8])+ .1, y = yIndex[8]-offset,hjust = 0,
             label = SI.seqCharge.SI$SeqCharge[8], angle = 45, color = SI.seqCharge.SI$color[8], fontface = 2) + theme_light() + labs(x = bquote( 'Log'[10]~ 'Precursor Intensity'), y ="Precursors")  + geom_histogram(bins = 100, fill = "transparent", color = "grey40") + theme(axis.title = element_text(size =22), axis.text = element_text(size = 20))
ggsave(paste0(FigOut,"SpikeIn_Intensity.pdf"), width = 7, height = 5)
```
...
#evi.tier.p.spike.m %>% dplyr::filter(SeqCharge == "_AYFTAPSSER_2") %>% ggplot(aes(log10(value))) + geom_histogram() + facet_grid(rows = vars(Raw.file), cols = vars(variable)) + coord_flip()

#Old Search Pattern
#patternIndStartRef <- c("Reporter.intensity.corrected.13","Reporter.intensity.corrected.12","Reporter.intensity.corrected.11","Reporter.intensity.corrected.5","Reporter.intensity.corrected.18","Reporter.intensity.corrected.17","Reporter.intensity.corrected.16","Reporter.intensity.corrected.15","Reporter.intensity.corrected.14","Reporter.intensity.corrected.13","Reporter.intensity.corrected.12","Reporter.intensity.corrected.11","Reporter.intensity.corrected.5","Reporter.intensity.corrected.18","Reporter.intensity.corrected.17","Reporter.intensity.corrected.16","Reporter.intensity.corrected.15","Reporter.intensity.corrected.14","Reporter.intensity.corrected.13","Reporter.intensity.corrected.12","Reporter.intensity.corrected.11","Reporter.intensity.corrected.5","Reporter.intensity.corrected.18","Reporter.intensity.corrected.17","Reporter.intensity.corrected.16","Reporter.intensity.corrected.15","Reporter.intensity.corrected.14","Reporter.intensity.corrected.13","Reporter.intensity.corrected.12","Reporter.intensity.corrected.11","Reporter.intensity.corrected.5","Reporter.intensity.corrected.18","Reporter.intensity.corrected.17","Reporter.intensity.corrected.16","Reporter.intensity.corrected.15","Reporter.intensity.corrected.14","Reporter.intensity.corrected.13","Reporter.intensity.corrected.12","Reporter.intensity.corrected.11")
...
for(i in 1:length(unique(evi.tier.p.spike$Raw.file))){
  ...
  #Rebuttal_CG.lim <- Rebuttal_CG_both.m.lim  %>% filter(Set ==unique(evi.tier.p.spike$Raw.file)[i])
  ...
  #evi_consist.single.m.lim <- evi_consist.single.m %>% filter(Channel %in% Rebuttal_CG.lim$RI) %>% select(logConcentration,normInt)
 # evi_consist.single.m.lim <- evi_consist.single.m %>% select(logConcentration,normInt)
 # eviMat <- as.matrix(evi_consist.single.m.lim)
  
 #  evi_consist.single.m.lim$logInt.cor <- ifelse(!is.finite(evi_consist.single.m.lim$normInt),NA,evi_consist.single.m.lim$normInt)
 # model = lm(logInt.cor ~ logConcentration - 1, dat = evi_consist.single.m.lim)

#  intercept <- 0
#  slope <- summary(model)[[4]][1]
#  rSq <- summary(model)[[8]]
  
#  model_outputs[i,1] <- unique(evi.tier.p.spike$Raw.file)[i]
#  model_outputs[i,2] <- intercept
#  model_outputs[i,3] <- slope
#  model_outputs[i,4] <- rSq
...
# Column and row normalization (borrowed from scp pipeline function for consistency)
cr_norm_log<-function(dat){
...
#---------------
# Fig. 5: Endocytosis Figure Components
#---------------
...
ShotgunComp <- read.delim(paste0(datIn,"EDF4_IMBR/shotunSC_noIMBR_evidence.txt"))
